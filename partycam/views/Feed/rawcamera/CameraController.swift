...
class CameraController: UIViewController, ObservableObject {
    enum Camera {
        case front, back
    }
    ...
    func takePhoto(_ completion: ((Photo) -> Void)? = nil) {
        ...
        processor.completion = { [unowned self] processor in
            ...
            if let settings = processor.settings {
                ...
                // Force remove all processors if settings are somehow missing
                print("⚠️ Processor settings were nil. Cleaning up all.")
                self.captureProcessors.removeAll()
            }
        }
...
extension PhotoCaptureProcessor: AVCapturePhotoCaptureDelegate {
    func photoOutput(_ output: AVCapturePhotoOutput,
        ...
        if let data = photo.fileDataRepresentation() {
            ...
            print("❌ fileDataRepresentation() returned nil in delegate")
        }
